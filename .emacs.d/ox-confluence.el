...
;; Notes/TODO's
;;  - Implement footnotes by
;;    - [x] converting the footnote list into a numbered list
;;    - [x] Use anchor in the list to make {fn:<N>}
;;    - [x] Where the footnote is used replace the [fn:<N>] with a superscript <N> and link to the anchor at #fn<N>
;;  - Add options to specify the space and title of an exported page
;;    - This will help with using the REST api to send
;;  - Add abiliity to post and update
;;  - [x] Unfill paragraphs
;;    - The browser can control line wrapping/scrolling
...
(org-export-define-derived-backend 'confluence 'ascii
  :translate-alist '((bold . org-confluence-bold)
		     ...
		     (footnote-reference . org-confluence-footnote-refernce)
  ...
  :options-alist '((:confluence-pub-page-title "CONFLUENCE_PUB_PAGE_TITLE" nil nil space)
                   (:confluence-pub-page-space "CONFLUENCE_PUB_PAGE_SPACE" nil nil space)
                   )
  :menu-entry
...
(defun org-confluence-footnote-definition (footnote-def contents info)
  (let ((footnote-number (org-element-property :label footnote-def))
        (footnote-text contents))
    (format "\{anchor:fn%s\}%s: %s"
            footnote-number
            footnote-number
            footnote-text)
    )
  )
(defun org-confluence-footnote-refernce (footnote-ref contents info)
  (let ((footnote-number (org-element-property :label footnote-ref)))
    (format "^[%s|#fn%s]^"
            footnote-number
            footnote-number)
    ))
...
(defun org-confluence-verbatim (verbatim contents info)
  (format "\{\{%s\}\}" (org-confluence--escape-brackets
...
(defun org-confluence--escape-brackets (string)
  "Replace brackes in STRING with escaped brackets"
  (with-temp-buffer
    (insert string)
    (goto-char (point-min))
